/**
 * Seller Orders Page
 * List and manage orders as a seller
 */
...
const statusConfig: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  [OrderStatus.CREATED]: { label: 'New Order', color: 'text-blue-600', bg: 'bg-blue-50', icon: Package },
  [OrderStatus.SHARIAH_VALIDATED]: { label: 'Validated', color: 'text-emerald-600', bg: 'bg-emerald-50', icon: FileCheck },
  [OrderStatus.AI_EVALUATED]: { label: 'AI Checked', color: 'text-purple-600', bg: 'bg-purple-50', icon: CheckCircle },
  ...
  [OrderStatus.FUNDED]: { label: 'Ready to Process', color: 'text-green-600', bg: 'bg-green-50', icon: CreditCard },
  [OrderStatus.PROCESSING]: { label: 'Processing', color: 'text-indigo-600', bg: 'bg-indigo-50', icon: RefreshCw },
  [OrderStatus.IN_TRANSIT]: { label: 'Shipped', color: 'text-orange-600', bg: 'bg-orange-50', icon: Truck },
  ...
  [OrderStatus.SETTLED]: { label: 'Payment Received', color: 'text-emerald-600', bg: 'bg-emerald-50', icon: DollarSign },
...
export default function SellerOrdersPage() {
  ...
  const { data: ordersData, isLoading, error, refetch } = useQuery<OrderListResponse>(
    ...
    () => apiClient.getMyOrders({ role_filter: 'seller', status: statusFilter || undefined, limit: 100 }),
  ...
  // Stats
  const pendingOrders = orders.filter((o) => o.status === OrderStatus.FUNDED).length;
  const processingOrders = orders.filter((o) => o.status === OrderStatus.PROCESSING || o.status === OrderStatus.IN_TRANSIT).length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50">
      ...
      <div className="fixed inset-0 opacity-30 pointer-events-none">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="seller-orders-pattern" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M30 0L60 30L30 60L0 30Z" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-emerald-200" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#seller-orders-pattern)" />
        </svg>
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">ETB {totalRevenue.toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value={OrderStatus.FUNDED}>Ready to Process</option>
                <option value={OrderStatus.PROCESSING}>Processing</option>
                <option value={OrderStatus.IN_TRANSIT}>Shipped</option>
                <option value={OrderStatus.DELIVERED}>Delivered</option>
                <option value={OrderStatus.SETTLED}>Payment Received</option>
        ...
        {isLoading ? (
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-12 shadow-sm border border-gray-200 text-center">
            ...
            <p className="text-gray-600 mb-6">Orders from buyers will appear here</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredOrders.map((order) => {
              ...
              const needsAction = order.status === OrderStatus.FUNDED;

              return (
                <Link
                  key={order.id}
                  href={`/seller/orders/${order.id}`}
                  className={`block bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border ${
                    needsAction ? 'border-amber-300 ring-2 ring-amber-100' : 'border-gray-200'
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          {needsAction && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-xs font-medium animate-pulse">
                              Action Required
                            </span>
                          )}
                        </div>
                        ...
                        <p className="text-sm text-gray-600 flex items-center gap-2">
                          ...
                          Buyer: {order.buyer?.business_name || order.buyer?.full_name || 'Unknown'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-gray-500">
                      ...
                      {order.expected_delivery_date && (
                        <span className="flex items-center gap-1">
                          ...
                          Deliver by {new Date(order.expected_delivery_date).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                    <span className="flex items-center gap-1 text-emerald-600 font-medium group-hover:gap-2 transition-all">
                      {needsAction ? 'Process Order' : 'View Details'}
